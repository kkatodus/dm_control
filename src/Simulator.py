...
class Simulator:
	def __init__(self, domain_name='stretch3', task_name='test', use_rt1=False):
		...
		self.raw_action_records = defaultdict(list)
		self.input_camera_type = 'head_feed'
		self.input_camera_options = ['head_feed', 'wrist_feed', 'third_person']
...
#############RT1 to Sim Interface ##############################
	...
	def translate_rt1_action_to_sim_actions(self, rt1_action):
		...
		rt1_pitch = rotation_delta[0]
		rt1_yaw = rotation_delta[1]
		rt1_roll = rotation_delta[2]
		...
		# w_left, w_right = self.diff_drive_inv_kinematics(1, omega)
		lift_height = map_RT1_output_to_mujoco_limits(world_vector_z, ACTUATOR_LIMITS['lift'])
		arm_extend = map_RT1_output_to_mujoco_limits(world_vector_x, ACTUATOR_LIMITS['arm'])
		...
		action[arm_actuator_idx] = arm_extend
		...
		action[gripper_actuator_idx] = gripper_opening
...
###############SIMULATION###########################	
	...
	def run_sim(self, duration=4):
		...
		policy_state = self.tfa_policy.get_initial_state(batch_size=1)
		...
		while self.env.physics.data.time < duration:
			...
			observation['natural_language_embedding'] = self.natural_language_command_embedding
			observation['image'] = timestep.observation[self.input_camera_type]
			tfa_time_step = ts.transition(observation, reward=np.zeros((), dtype=np.float32))

			policy_step = self.tfa_policy.action(tfa_time_step, policy_state)
	...
	def record_rt1_actions(self, rt1_action):
		for action_key in RT1_RAW_ACTION_KEYS:
				...
				for dim in range(RT1_RAW_ACTION_KEYS2DIMS[action_key]):
					self.raw_action_records[action_key+'_'+str(dim)].append(one_action[dim])

	def plot_rt1_actions(self):
		fig, ax = plt.subplots(len(self.raw_action_records), figsize=(20, 50))
		for idx, (key, value) in enumerate(self.raw_action_records.items()):
			ax[idx].plot(value)
